...
- **17 new effects.** Object motion: Afterimage (a fading ghost trail), Jelly Wobble, Squash Land, Stretch, Wobble Rotation, Hop, Breathe, and Heartbeat. Impact: Hit Spark (a directional spark spray), Slash Arc, Crack, and Sparkle. Physics: Explosion Push (a radial shove on nearby RigidBody2Ds) and Gravity Shift. Screen, text, and audio: Glow Pulse, Rich Text Emphasis (wave / shake / rainbow on a RichTextLabel), and Tremolo. That brings the effect library to 117, and each new one carries its 2D / 3D dimension badge.
- **Edit an effect's properties right in the Inspector.** Every effect row in the sequence now has a foldout that opens the effect's own fields inline - sliders, dropdowns, colour pickers, node paths - with full undo/redo, instead of sending you off to a separate dock.
- **Preview settings under Project Settings > Juicee.** Toggle or recolour the border that marks the editor-preview area, and hide the hover info panel or just its description line.
- **Loop can run forever.** A "loop forever" option repeats the connected chain until the scene changes, for idle and ambient loops.

### Changed

- **The JuiceePlayer inspector was redrawn.** Compact rows in the spirit of Unity FEEL and GameFeelFlow: a quiet category tint, the effect's category and duration shown inline, a 2D / 3D badge, an "Effects (N)" count, and reorder / remove controls that surface on hover. It uses the editor's own font and colours so it reads as part of Godot instead of a bolted-on panel.
- **The graph editor and hover preview got a polish pass.** Blocks carry a category tint and their duration, the toolbar uses native editor icons, the properties panel has bold section headers and a proper empty state, and everything moved to the native editor font instead of hardcoded sizes.

### Fixed

- **Camera Follow did nothing in the editor preview.** It now pans the camera to a temporary offset target so you can see the follow without launching the game.
- **Trail, Pulse, Pop In and Scale placed or scaled things wrong in some setups.** Ghosts and spawned nodes are positioned after they enter the tree now (so a scene root away from the origin no longer doubles the offset), and Control-based effects scale from their centre instead of stretching out of a corner.
- **The editor-preview overlay could get stuck on screen.** Spamming Preview, or switching to a different effect, could leave the preview border stranded. Every overlay now carries a marker and each preview clears leftovers first, so nothing survives a switch.
